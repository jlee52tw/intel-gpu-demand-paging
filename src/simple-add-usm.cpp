...
int main() {
  ...
  try {
    ...
    cout << "=== SYCL USM Example - Parallel Iota ===" << "\n";
    cout << "Running on device: "
         << q.get_device().get_info<info::device::name>() << "\n";
    cout << "Array size: " << array_size << "\n\n";

    cout << "[CPU] Allocating USM shared memory for sequential array..." << "\n";
    int *sequential = malloc_shared<int>(array_size, q);
    cout << "[CPU] Allocating USM shared memory for parallel array..." << "\n";
    ...
    cout << "[CPU] Memory allocation successful!\n\n";

    // Sequential iota.
    cout << "[CPU] Computing sequential iota (for-loop on CPU)..." << "\n";
    for (size_t i = 0; i < array_size; i++) sequential[i] = value + i;
    cout << "[CPU] Sequential computation complete!\n\n";

    // Parallel iota in SYCL.
    cout << "[GPU] Launching parallel kernel on device..." << "\n";
    cout << "[GPU] Executing " << array_size << " work items in parallel..." << "\n";
    IotaParallel(q, parallel, array_size, value);
    cout << "[GPU] Kernel execution complete!\n\n";

    // Verify two results are equal.
    cout << "[CPU] Verifying GPU results against CPU results..." << "\n";
    for (size_t i = 0; i < array_size; i++) {
      if (parallel[i] != sequential[i]) {
        cout << "[CPU] Verification FAILED at index " << i << "!\n";
        cout << "  Expected: " << sequential[i] << ", Got: " << parallel[i] << "\n";
        return -1;
      }
    }
    cout << "[CPU] Verification PASSED! All " << array_size << " elements match!\n\n";

    int indices[]{0, 1, 2, (array_size - 1)};
    constexpr size_t indices_size = sizeof(indices) / sizeof(int);

    // Print out iota result.
    cout << "Sample results (index: index + value = result):" << "\n";
    ...
    cout << "[CPU] Freeing USM shared memory..." << "\n";
    free(sequential, q);
    free(parallel, q);
    cout << "[CPU] Memory cleanup complete!" << "\n";
  } catch (std::exception const &e) {
